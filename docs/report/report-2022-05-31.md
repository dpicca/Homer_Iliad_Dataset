...
Les données utilisées dans le cadre de ce travail ont été collectées par les étudiants du cours « Bases de données » de l’université de Lausanne, en automne 2021. Chaque étudiant était assigné un chapitre de la traduction Française de l’Iliade d’Homère qu’il se devait d’annoter. Chaque chapitre avait un total de 4 étudiants qui lui était assigné, chacun travaillant individuellement. Pour ce faire, les étudiants utilisaient le logiciel LabelStudio afin de parcourir chaque paragraphe de leur chapitre.
...
--La partie du paragraphe dans laquelle chaque émotion apparait (Speech)
Les étudiants avaient accès à 8 émotions différentes pour annoter leurs paragraphes : Ardeur, Calme, Joie, Plaisir, Dégoût, Tristesse, Colère, et Peur. Une fois que l’entièreté du chapitre était annotée, les étudiants étaient invités à convertir leur travail au format JSON.
...
Bien que la plupart des données ont correctement été récupérées, certaines erreurs ont toutefois rendu leur utilisation plus complexe que prévu.
Certains annotateurs, dans l’espoir de gagner du temps, ont omis de créer une entrée « Speaker » par paragraphe, préférant à la place en créer une seule et de la lier à multiples paragraphes. Les entrées « Speaker » avaient donc, au lieu d’un identifiant unique, une portée de chiffre (Par exemple, [37-41], ou encore [37, 38, 40, 45, 49]) ce qui s’est avéré difficile à déchiffrer au travers de code Python. La modification de fichiers JSON étant difficile sans le logiciel originel, ces entrées ont été manuellement modifiées pour transformer les portées en chiffres unique (Par exemple, [37-41] devenant [37]).
...
Pour ces nombreuses raisons, il est important de souligner que les données utilisées dans le cadre de ce projet ne sont par parfaites, et qu’il est donc normal de s’attendre à des incohérences légères dans les résultats obtenus. Toutefois, le fait que chaque chapitre était assigné a de multiples étudiant devrait avoir limité ces problèmes.

## Traitement des données

Afin de traiter ces données, nous avons converti chaque fichier JSON en un DataFrame à l’aide de la libraire Python « Pandas ».
Une fois cette courte étape terminée, les fichiers JSON produits par LabelStudio étant riches en information, la première partie du traitement des données consistait à ne garder que les données nécessaires à ce projet :
- Data_type (Emotions/Speaker/Speech)
...
- Speaker (La personne disant la phrase en question)
...
# La problématique des mots d'une phrase

L'ontologie de départ prévoyait de mettre en relation un mot avec une émotion par l'intermédiaire du sens du mot. Ainsi il existe une sous-classe de mot (*Word*) pour chaque émotion, par exemple pour la joie avec *Joy_Words*. En s'appuyant sur la définition de *Lexical Entry* tiré l'ontologie Ontolex-Lemon, nous pourrions ainsi donner du sens aux mots via le *Lexical Concept* puis les ontologies existantes, tel BabelNet ou WordNets.

Mais cela ne correspond pas à nos données. En effet, celles-ci sont constituées d'une ou plusieurs phrases où l'émotion a déjà été attribuée par un annotateur.

Nous avons alors opté pour utiliser l'ontologie NLP Interchange Format (NIF) qui permet l'usage de morceaux de textes et qui est utilisé pour l'annotation de corpus. Nos extraits annotés (*Speech*) ont alors été déclarés comme équivalents à la classe *String* de NIF. Au sein de cette classe, nous pouvons stocker le morceau de texte, grâce à la propriété d'objet **nif:isString** liée à la sous-classe *Context*. Il est alors possible de noter le débuts et la fin de chaque extrait.
...
![Les 16 combinaisons de rôles observées ainsi que leur type Myers-Briggs correspondant. Les flèches indiquent le flux naturel de la pensée, et les émotions dominantes conscientes sont en gras et commencent par une lettre majuscule[^6]](images/emotion-MBTI.png){#fig:emotion-mbti}
...
![Les règles SWRL[^6]](images/rules.png){#fig:rules-swrl}

# Utilisation des données dans l'ontologie

WIP. Notes préparatoires :

pickle pour faire passer les dataframes entre nettoyage et importation.

Owlready2, incompatible turtle. Problème avec notre ontologie et avec NIF.

Importation manuelle d'un speaker et d'un speech test. Révélateur de tous les problèmes ! On aurait dû rapidement importer un speech test.

Bilan : pas d'importations automatiques. éventuellement subset manuel.
...
2. https://psychologyofemotion.wordpress.com/tag/myers-briggs/
